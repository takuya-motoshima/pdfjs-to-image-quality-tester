...
- **レイアウト**: 左ペイン (320px固定) と右ペイン (flex) の2カラム構成
- **自動再レンダリング**: 設定変更時に500msのdebounce処理で自動的に`renderAllPages()`を実行
- **シンプルなUI**: カード不使用のフラットデザイン、セクション見出しは`<h6>`タグで表現
- **エラー表示**: シンプルな`alert()`による通知

## コード変更時の考慮事項

- UI変更時は左ペイン (320px固定) と右ペイン (flex) のレイアウトバランスを維持
- 新しい品質パラメータ追加時は以下を更新:
  - `renderPage()` 関数
  - `autoRerender()` 関数のイベントリスナー登録
- レスポンシブデザイン (768px breakpoint) を考慮
- 設定変更の自動反映はdebounceで制御（連続変更時の負荷軽減）
